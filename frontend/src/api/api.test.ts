...
// wailsjs 绑定在 node 环境不可用，用 vi.mock 隔离（本文件不触达真实后端）。
import { beforeEach, describe, expect, it, vi } from 'vitest'

// EventsOn 按 Wails 约定返回「仅取消该监听」的函数；用 spy 记录以便断言退订行为。
...
vi.mock('../../wailsjs/go/main/App', () => ({ StartHashTask: mocks.StartHashTask }))
