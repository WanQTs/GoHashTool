...
// 所有与 Go 后端的交互都必须经过本模块，视图不直接引用 wailsjs。
...
export const pickFiles = () => call(App.PickFiles())
export const pickFolder = () => call(App.PickFolder())
export const pickManifestFile = () => call(App.PickManifestFile())
export const pickSavePath = (defaultName: string, filterName: string, pattern: string) =>
  call(App.PickSavePath(defaultName, filterName, pattern))
...
export function createTaskSession() {
  ...
  let minSeqAtStart = 0
  // EventsOn 返回的单个监听取消函数；destroy 时必须只退订本会话的监听，
  // 不能用全局 EventsOff（会把其他视图会话的同名监听一并移除）。
