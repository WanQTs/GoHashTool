...
param(
    [string]$ExePath = "build\bin\gohash.exe",
...
# 注：GUI 子系统进程必须加 -NoNewWindow（走 CreateProcess），否则 ShellExecute 路径的
# ExitCode 偶发为空（PowerShell + 瞬时退出的已知问题）。
$selftest = Start-Process -FilePath $ExePath -ArgumentList "--selftest" -Wait -PassThru -NoNewWindow
