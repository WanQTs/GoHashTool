...
- **System integration**: single instance (relaunching focuses the running window and forwards the file); optional checksum manifest file associations (explicit on/off switch in Settings, off by default — double-click .sha256/.md5 etc. manifests to open Batch Verify directly; per-user HKCU registration, never hijacks extensions owned by other apps, path self-heals after the exe is moved); taskbar flash when a task finishes while the window is in the background; always-on-top toggle; native row context menu (copy hash / copy path / show in Explorer).
