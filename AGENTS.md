...
openwith.go              「打开方式」接入：清单扩展名识别（assocExts 注册用 / openWithExts 识别用，
                         ...
                         文件关联三绑定（GetFileAssocStatus / RegisterFileAssociations /
                         UnregisterFileAssociations，非 Windows 返回 unsupported_platform）
...
fileassoc.go             文件关联纯逻辑（可单测）：progIDFor / assocCommandFor /
                         planAssocWrite（未关联写、他人占用跳、自有陈旧自愈）/
                         planAssocHeal（只动自有陈旧，未关联绝不写）/ planAssocRemove（只删自有）
fileassoc_windows.go     关联的注册表 IO（读 HKCR 合并视图、写删只走 HKCU\Software\Classes）：
                         fileAssocCount 状态计数、registerFileAssoc（注册含自愈）、
                         unregisterFileAssoc（按子键顺序删 ProgID 树）、healFileAssoc（启动自愈）、
                         SHChangeNotify 刷新外壳；fileassoc_other.go 为非 Windows 空实现
                         （fileAssocSupported=false，保持可交叉编译）
...
- 设置入口为顶栏齿轮弹层组件 `components/settings-menu.vue`（`app.vue` 内挂载于 NMessageProvider 之下，可用 useMessage）：目前是文件关联显式开关——挂载时 `getFileAssocStatus` 取勾选态（`Result.count` 为已关联扩展名数），切换调 `registerFileAssociations`/`unregisterFileAssociations`，失败回滚勾选并 toast；后续新增设置项都进这个弹层。
...
- 文件关联为**设置里的显式开关（默认关）**：注册/解除只写/删 `HKCU\Software\Classes` 下本应用自有 ProgID（免管理员），写前读 HKCR 合并视图判断，被其他程序占用的扩展名跳过不劫持；`.txt` 只参与识别、不参与注册；启动时仅自愈「自有但 exe 路径已陈旧」的条目，未注册机器零注册表写入。
- 发布物只有 `wails3 build` 产出的单文件 exe（可选 UPX 压缩），无安装器、无服务端部署流程。
- 备份说明：v2→v3 迁移前的完整源码快照在本仓库的 `v2` 分支（回滚参考用，不参与 main 开发）。
