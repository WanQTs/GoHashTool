...
- **系统集成**：单实例运行（重复启动聚焦已有窗口并转交文件）；清单文件关联可选注册（设置里一键注册/解除，默认关；双击 .sha256/.md5 等清单直接打开批量校验；HKCU 免管理员、不劫持已占用扩展名、exe 移动后自动更新路径）；任务完成时窗口不在前台则任务栏闪烁提醒；窗口可一键置顶；结果行右键菜单（复制哈希/复制路径/在资源管理器中显示）。
